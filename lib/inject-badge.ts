/**
 * Injects the bapXcode badge script into HTML content before the closing </body> tag.
 ...
 */
export function injectbapXcodeBadge(html: string): string {
  const badgeScript = '<script src="https://huggingface.co/bapxcode/bapxcode-badge.js"></script>';
  
  // Remove any existing badge script to avoid duplicates
  const cleanedHtml = html.replace(
    /<script\s+src=["']https:\/\/bapxcode\.hf\.co\/bapxcode-badge\.js["']\s*><\/script>\s*/gi,
    ''
  );
