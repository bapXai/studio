(function() {
  ...
  if (document.getElementById('bapxcode-badge-wrapper')) {
    return;
  }

  // Don't show badge on huggingface.co/bapxcode
  const hostname = window.location.hostname;
  const pathname = window.location.pathname;
  if (hostname === 'huggingface.co' && pathname.startsWith('/bapxcode')) {
    return;
  }
  ...
  style.textContent = `
    ...
    #bapxcode-badge-wrapper i::before {
      content: "";
      position: absolute;
      left: 50%;
      top: 50%;
    ...
    #bapxcode-badge-wrapper:hover i::before {
      opacity: 1;
      animation: bapxcode-spin 3s linear infinite;
    }
  `;
  ...
  link.href = 'https://huggingface.co/bapxcode';
  link.target = '_blank';
  link.rel = 'noopener noreferrer';
  
  // Create icon placeholder
  const icon = document.createElement('img');
  icon.src = 'https://huggingface.co/bapxcode/logo.svg';
  icon.alt = 'bapXcode';
  ...
  // Create text
  const text = document.createTextNode('Made with bapXcode');
