...
export const useAi = (onScrollToBottom?: () => void) => {
  ...
  const createNewProject = async (prompt: string, htmlPages: Page[], projectName: string | undefined, isLoggedIn?: boolean, userName?: string) => {
    if (isLoggedIn && userName) {
      try {
        const uploadRequest = await fetch(`/bapxcode/api/me/projects/${userName}/new/update`, {
          method: "PUT",
          body: JSON.stringify({
            pages: htmlPages,
            commitTitle: prompt,
  ...
  const callAiNewProject = async (prompt: string, enhancedSettings?: EnhancedSettings, redesignMarkdown?: string, isLoggedIn?: boolean, userName?: string) => {
    ...
    try {
      const request = await fetch("/bapxcode/api/ask", {
        method: "POST",
        body: JSON.stringify({
          prompt,
          provider,
      ...
      if (request && request.body) {
        ...
        const read = async (): Promise<any> => {
          ...
          if (done) {
            ...
            try {
              const uploadRequest = await fetch(`/bapxcode/api/me/projects/${namespace ?? 'unknown'}/${repoId ?? 'unknown'}/update`, {
                method: "PUT",
                body: JSON.stringify({
                  pages: mergedPages,
                  commitTitle: prompt,
