...
bapXcode is now available as an MCP (Model Context Protocol) server, enabling AI assistants like Claude to create websites directly using natural language.

## Two Ways to Use bapXcode MCP
...
**No installation required!** Use bapXcode's hosted MCP server.
...
```json
{
  "mcpServers": {
    "bapxcode": {
      "url": "https://huggingface.co/bapxcode/api/mcp",
      "transport": {
        "type": "sse"
      },
      "headers": {
...
> "Create a portfolio website using bapXcode. Include a hero section, about section, and contact form."
...
```json
{
  "mcpServers": {
    "bapxcode-local": {
      "command": "node",
      "args": ["/absolute/path/to/bapxcode-v3/mcp-server/dist/index.js"],
      "env": {
        "HF_TOKEN": "hf_your_token_here",
        "BAPXCODE_API_URL": "https://huggingface.co/bapxcode"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "bapxcode-local": {
      ...
      "args": ["/absolute/path/to/bapxcode-v3/mcp-server/dist/index.js"],
      "env": {
        "BAPXCODE_AUTH_COOKIE": "your-session-cookie",
        "BAPXCODE_API_URL": "https://huggingface.co/bapxcode"
      }
    }
  }
...
1. Log in to https://huggingface.co/bapxcode
2. Open Developer Tools (F12)
3. Go to Application → Cookies
4. Copy the session cookie value
5. Set as `BAPXCODE_AUTH_COOKIE` in the config
...
Creates a new bapXcode project with HTML/CSS/JS files.

**Parameters:**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `title` | string | No | Project title (defaults to "bapXcode Project") |
...
```json
{
  ...
  "projectUrl": "https://huggingface.co/bapxcode/username/project-name",
...
### Simple Landing Page
> "Create a modern landing page for my SaaS product using bapXcode. Include a hero section with CTA, features grid, and footer. Use gradient background."

### Portfolio Website
> "Build a portfolio website with bapXcode. I need:
...
### Blog Homepage
> "Create a blog homepage using bapXcode. Include:
...
> "Make an analytics dashboard with bapXcode:
> - Sidebar navigation
...
```bash
curl -X POST https://huggingface.co/bapxcode/api/mcp \
...
> "Create a portfolio website with bapXcode"

Instead of:
> "Create a portfolio website with bapXcode using token `hf_xxxxx`"
...
- bapXcode does not store your token
...
- Or use `BAPXCODE_AUTH_COOKIE` if you prefer session-based auth
...
Claude Desktop
 ...
 (HTTP Request)
      ↓
huggingface.co/bapxcode/api/mcp
      ↓
...
Local MCP Server
      ↓
 (HTTP to bapXcode API)
      ↓
huggingface.co/bapxcode/api/me/projects
      ↓
New Space Created
```
...
Both use the same core bapXcode logic for creating projects - no duplication!
...
- [bapXcode Documentation](https://huggingface.co/bapxcode)
